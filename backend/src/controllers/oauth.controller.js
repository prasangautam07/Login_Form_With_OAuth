...
export const googleOAuth = async (req, res)=>{
    try{
        ...
        if (!googleAccessToken) return res.status(400).json({ message: "No access token" });
...
export const githubOAuth = async (req, res)=>{
    try{
        ...
        if(!email){
            ...
            const primaryEmail = emailsResponse.data?.find(emailObj => emailObj.primary && emailObj.verified);
            email = primaryEmail ? primaryEmail.email : `${name.toLowerCase().replace(/\s/g, '')}@github.com`;
        }
